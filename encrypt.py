...
####################################################  USER INPUT  #############################################################
	...
	####################################ROH############################################	
	df_Cancel.loc[df_Cancel['CountryName'] != 'United States', 'Base Rate'] = 'UZ'
	...
	for i in range(5):
		...
		if sea[i].empty:
			...
			if lra=='2':
				if ol=='0':
					res[i].loc[sea[i]['AcceptedRateType'] == 'LRA', '1 Person Rate']=  np.repeat(sea[i]['Preferred_LRA_S'+str(i+1)+'_SGL'], lens[i])
					res[i].loc[sea[i]['AcceptedRateType'] == 'Non-LRA', '1 Person Rate']=  np.repeat(sea[i]['Preferred_Non_LRA_S'+str(i+1)+'_SGL'], lens[i])
					res[i]['2 Person Rate']= 0
				if ol=='1':
					res[i]['1 Person Rate']= 0
					res[i].loc[sea[i]['AcceptedRateType'] == 'LRA', '1 Person Rate']=  np.repeat(sea[i]['Preferred_LRA_S'+str(i+1)+'_DBL'], lens[i])
					res[i].loc[sea[i]['AcceptedRateType'] == 'Non-LRA', '1 Person Rate']=  np.repeat(sea[i]['Preferred_Non_LRA_S'+str(i+1)+'_DBL'], lens[i])
				if ol=='2':
					res[i].loc[sea[i]['AcceptedRateType'] == 'LRA', '1 Person Rate']=  np.repeat(sea[i]['Preferred_LRA_S'+str(i+1)+'_SGL'], lens[i])
					res[i].loc[sea[i]['AcceptedRateType'] == 'Non-LRA', '1 Person Rate']=  np.repeat(sea[i]['Preferred_Non_LRA_S'+str(i+1)+'_SGL'], lens[i])
					res[i].loc[sea[i]['AcceptedRateType'] == 'LRA', '1 Person Rate']=  np.repeat(sea[i]['Preferred_LRA_S'+str(i+1)+'_DBL'], lens[i])
	...
	for i in range(5):
		...
		if seafd[i].empty:
			...
			refd[i]['1 Person Rate']= np.repeat(seafd[i]['BD'+str(i+1)+'_RT1_SGL'], lensfs[i])
			refd[i]['CountryName']=np.repeat(dfFMS[i]['CountryName'], lensfs[i])
			refd[i]['1 Person Rate'].fillna(seafd[i]['BD'+str(i+1)+'_RT2_SGL'], inplace=True)
			refd[i]['1 Person Rate'].fillna(seafd[i]['BD'+str(i+1)+'_RT3_SGL'], inplace=True)
	...
	Cancel_Policy['Hold Code']='6PM'
	Cancel_Policy.loc[Cancel_Policy['Cancel Code'] == "16:00", 'Hold Code'] = '4PM'
	...
	for name,group in date_group:
		group['overlap'] = (group['End_Date'].shift()-group['Begin_Date']) > timedelta(0)
		if (group['overlap']==True).any():
			qa.append(group.Resort.unique())
			repeat=1
			w=1
			string="Resort Number"+group.Resort.unique()+ " "  +group['Room Type'].unique()+ "Has overlapping dates in Main Tab"
	...
	if w==0:
		...
		Hotel_Details_Final['Flat/Percentage'] = None
