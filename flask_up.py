...
from flask import Flask, flash, request, redirect, url_for,render_template,send_from_directory,make_response,current_app,session
import encrypt
...
def hello():
	...
	if request.method == 'GET':
		...
		if "Add" in request.form:
			...
			return redirect(url_for('userinput',GDS_code=GDS_code,Multi_code=Multi_code,Sabre=Sabre,Worldspan=Worldspan,Amaedus=Amaedus,Galileo=Galileo,Web=Web,f=f,fd=fd))
		else:
			Multi_code=None
			f=request.args.get('f')
			fd=request.args.get('fd')
			...
			return redirect(url_for('backend',GDS_code=GDS_code,Multi_code=Multi_code,Sabre=Sabre,Worldspan=Worldspan,Amaedus=Amaedus,Galileo=Galileo,Web=Web,f=f,fd=fd))
...
def backend():
	...
	df_Main.loc[df_Main['CountryName'] == 'United States', 'BWIRateCode'] = ''
	c=pd.DataFrame({'Corp Acct#': df_Main['CRSIdentifier'],'Resort': df_Main['CRSHotelID'],'BWI Rate Code': df_Main['BWIRateCode'],'GDS Rate Codes': GDS_code,'Multi Rate Code': Multi_code,'Begin Date': today,'End Date': '12/31/2099','Sabre' : Sabre,'Worldspan' : Worldspan,'Amaedus' : Amaedus,'Galileo' :Galileo,'Web':Web,'RoomDescription': df_Main['RoomDescription']})
...
def userinput():
	...
	d=f.to_json(orient='split')
	#encrypt.etl(d)
	return redirect(url_for('hello',f=d,fd=fd))
################################################################################################################################
